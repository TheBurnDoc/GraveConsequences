...
function NewStick( Props )
        ...
        function Group:move(Obj, maxSpeed, rotate)
                ...
                local x = Obj.x + Cos( Rad(self.angle-90) ) * (maxSpeed * self.percent)
                local y = Obj.y + Sin( Rad(self.angle-90) ) * (maxSpeed * self.percent)
                
                if x < display.contentWidth - 16 and x > 16 then
                    Obj.x = x
                end
                
                if y < display.contentHeight - 16 and y > 16 then
                    Obj.y = y
                end
        end
